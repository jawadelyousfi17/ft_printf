...
async function checkFilesByPath(argsl) {
    const filesToCheckFullPath = argsl.map(fileName => currentDirectory + '/' + fileName);
    ...
    for (let index = 0; index < filesToCheckFullPath.length; index++) {
        ...
        if (!exists) {
            console.log(yellow,`\r⚠️  ${argsl[index]}     is MISSING`,reset);
            err = true;
        }
    }
    return err;
...
module.exports = { checkFilesByPath }